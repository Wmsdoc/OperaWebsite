    ...
    <CommonPage title="戏曲音频列表">
      ...
      <n-modal v-model:show="showModal" preset="dialog" title="详情">
        <div style="display: flex; justify-content: center; align-items: center">
          <audio :src="details.downloadUrl" controls></audio>
        </div>
      ...
      <n-modal v-model:show="dialogVisible" preset="dialog" title="编辑">
        ...
        >
          ...
          <n-form-item label="音频简介" path="audioInfo">
            <n-input
              v-model:value="editForm.audioInfo"
              type="textarea"
              maxlength="200"
              show-count
            />
          </n-form-item>
          <n-form-item label="音频类型" path="typeName">
            <n-select
      ...
      return [
        h(
          NButton,
          {
            ...
            type: 'primary',
            secondary: true,
            onClick: () => handleView(row.audioId),
          },
        ...
        h(
          NButton,
          {
            ...
            onClick: () => handleEdit(row.audioId),
          },
        ...
        h(
          NButton,
          {
            ...
            type: 'error',
            style: 'margin-left: 15px;',
            onClick: () => handleDelete(row.audioId),
          },
          {
            default: () => '删除',
...
const handleExamine = (row) => {
  if (isNullOrUndef(row.audioId)) return
  row.examine = true
  let formData = new FormData()
  formData.append('audioId', row.audioId)
...
const handleView = (audioId) => {
  api.getPostById(audioId).then((res) => {
...
const handleEdit = (audioId) => {
  ...
  editForm.filename = ''
  editForm.audioInfo = ''
  api.getAudioById(audioId).then((res) => {
    editForm.audioId = res.data.audioId
    editForm.filename = res.data.filename
    editForm.audioInfo = res.data.audioInfo
...
const handleDelete = (audioId) => {
  $dialog.confirm({
    content: '确认删除？',
    confirm() {
      api.deletePost(audioId).then((res) => {
        if (res.code === 200 && res.data === true) {
          console.log(res)
...
const handleUpdate = () => {
  let formData = new FormData()
  formData.append('audioId', editForm.audioId)
  formData.append('filename', editForm.filename)
  formData.append('audioInfo', editForm.audioInfo)
